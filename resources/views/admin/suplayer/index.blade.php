    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered tablestriped" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    @foreach($suplayer as $supp)
                    <tr>
                        <td>{{$supp->kd_supp}}</td>
                        <td>{{$supp->nm_supp}}</td>
                        <td>{{$supp->alamat}}</td>
                        <td>{{$supp->telpon}}</td>
                        <td align="center"><a href="suplayer/edit/{{$supp->kd_supp}}" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm">
                                <i class="fas fa-edit fa-sm text-white-50"></i>Edit</a>
                            <a href="suplayer/hapus/{{$supp->kd_supp}}" onclick="return confirm('Yakin Ingin menghapus data?')" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm">
